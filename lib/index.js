    ...
    if (cmdOptions.createDoc) {
        ...
        folders.forEach(function (folder) {
            if (fs.existsSync(path.join(cmdOptions.templateLocation, folder, 'template.json'))) {
                docGeneration_1.createDoc(path.join(cmdOptions.templateLocation, folder), cmdOptions.markdownOutputLocation, cmdOptions.templateJsonFileName, cmdOptions.vsCodePrefix, cmdOptions.markdownFileName, cmdOptions.renderWithoutPrefixes);
            }
        });
    }
