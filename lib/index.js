...
const createDocs = (folders, cmdOptions, fullPath) => {
    folders.forEach(function (folder) {
        ...
        if (fs.existsSync(path.join(fullPath, folder, 'template.json'))) {
            docGeneration_1.createDoc(path.join(fullPath, folder), cmdOptions.markdownOutputLocation, cmdOptions.templateJsonFileName, cmdOptions.vsCodePrefix, cmdOptions.markdownFileName, cmdOptions.renderWithoutPrefixes);
        }
    });
};
